# Phase 5 — A1 Full Curriculum Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Expand the A1 level from its current 1 unit / 3 lessons to the full curriculum target of 12 units × 4 lessons (48 lessons total), following the topic outline in `docs/superpowers/specs/2026-09-15-phase5-a1-curriculum-design.md`.

**Architecture:** All new content is added to `prisma/seed.ts`, the existing single source of content (re-run via `npx prisma db seed`). Each task inserts one new `Unit` + its 4 `Lesson`s + `Exercise`s + `VocabWord`s, following the exact object-literal shape already used for the existing A1 unit (`Begrüßung`). No schema, API, or UI changes — the admin panel, `/learn` pages, and `ExerciseRunner` already render whatever is in the database.
...
- `FILL_IN_BLANK`/`SHORT_ANSWER` `accepted` arrays only need the canonical (umlaut-containing) spelling — `normalizeGermanText` in `src/lib/exerciseChecking.ts` lowercases and maps `ä→ae`, `ö→oe`, `ü→ue`, `ß→ss` on both sides before comparing, so ASCII-substitute duplicates are redundant.
- New `Unit`s get `levelId: a1.id` and ascending `order` values (2 through 12) so `units[0]` (via `orderBy: { order: 'asc' }` in `getUnitsForLevel`/`prisma.unit.findMany`) always stays the existing `Begrüßung` unit — this is required by `tests/integration/learn.test.ts`'s MATCHING test, which indexes `units[0].lessons[1]`.
...
- Commit after every task with a `feat:`-prefixed message, one commit per unit (matching the granularity of the Phase 3 `"feat: seed badges and vocab words"` precedent).
- Insertion anchor: every new unit block is inserted immediately before the line `  // --- A2: Vergangenheit (1 sample lesson) ---` in `prisma/seed.ts`. That comment is never modified, so it stays a stable, unique anchor across all 12 tasks.

---

### Task 1: Add Lesson 4 to the existing `Begrüßung` unit

**Files:**
- Modify: `prisma/seed.ts`
- Modify: `tests/integration/seedContent.test.ts`
- Modify: `tests/integration/learn.test.ts`

**Interfaces:**
- Produces: `a1Lesson4` (local variable in `prisma/seed.ts`, not exported — no other task depends on this specific variable name, only on the resulting DB row count).

- [ ] **Step 1: Insert Lesson 4 into `prisma/seed.ts`**

Find this exact block (the end of `a1Lesson3`'s exercises, immediately followed by the A2 comment):
...
- [ ] **Step 2: Update `tests/integration/seedContent.test.ts`**
...
- [ ] **Step 3: Update `tests/integration/learn.test.ts`**
...
(`a1?.unitCount` stays `toBe(1)` — unchanged this task, since no new `Unit` row is added yet.)
...
Run: `npx vitest run tests/integration/seedContent.test.ts tests/integration/learn.test.ts`
Expected: PASS.

- [ ] **Step 6: Lint and commit**

Run: `npx eslint prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts`

```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: add Begrüßung lesson 4 (sich vorstellen)"
```

---

### Task 2: Seed A1 Unit 2 — Artikel & Nomen
...
- [ ] **Step 1: Insert the unit into `prisma/seed.ts`**

Find the anchor line `  // --- A2: Vergangenheit (1 sample lesson) ---` and insert this block immediately before it:
...
Change `expect(units).toHaveLength(1)` to `expect(units).toHaveLength(2)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(1)` to `expect(a1?.unitCount).toBe(2)`.
Change `expect(units).toHaveLength(1)` to `expect(units).toHaveLength(2)`.
(`expect(units[0].lessons).toHaveLength(4)` stays unchanged — `units[0]` is still `Begrüßung`.)
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 2 - Artikel & Nomen"
```

---

### Task 3: Seed A1 Unit 3 — Personalpronomen & Präsens
...
Insert immediately before `  // --- A2: Vergangenheit (1 sample lesson) ---`:
...
Change `expect(units).toHaveLength(2)` to `expect(units).toHaveLength(3)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(2)` to `expect(a1?.unitCount).toBe(3)`.
Change `expect(units).toHaveLength(2)` to `expect(units).toHaveLength(3)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 3 - Personalpronomen & Präsens"
```

---

### Task 4: Seed A1 Unit 4 — Familie & Possessivpronomen
...
Change `expect(units).toHaveLength(3)` to `expect(units).toHaveLength(4)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(3)` to `expect(a1?.unitCount).toBe(4)`.
Change `expect(units).toHaveLength(3)` to `expect(units).toHaveLength(4)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 4 - Familie & Possessivpronomen"
```

---

### Task 5: Seed A1 Unit 5 — Zahlen, Uhrzeit & Alltag
...
Change `expect(units).toHaveLength(4)` to `expect(units).toHaveLength(5)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(4)` to `expect(a1?.unitCount).toBe(5)`.
Change `expect(units).toHaveLength(4)` to `expect(units).toHaveLength(5)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 5 - Zahlen, Uhrzeit & Alltag"
```

---

### Task 6: Seed A1 Unit 6 — Akkusativ
...
Change `expect(units).toHaveLength(5)` to `expect(units).toHaveLength(6)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(5)` to `expect(a1?.unitCount).toBe(6)`.
Change `expect(units).toHaveLength(5)` to `expect(units).toHaveLength(6)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 6 - Akkusativ"
```

---

### Task 7: Seed A1 Unit 7 — Modalverben
...
Change `expect(units).toHaveLength(6)` to `expect(units).toHaveLength(7)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(6)` to `expect(a1?.unitCount).toBe(7)`.
Change `expect(units).toHaveLength(6)` to `expect(units).toHaveLength(7)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 7 - Modalverben"
```

---

### Task 8: Seed A1 Unit 8 — Trennbare Verben & Alltag
...
Change `expect(units).toHaveLength(7)` to `expect(units).toHaveLength(8)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(7)` to `expect(a1?.unitCount).toBe(8)`.
Change `expect(units).toHaveLength(7)` to `expect(units).toHaveLength(8)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 8 - Trennbare Verben & Alltag"
```

---

### Task 9: Seed A1 Unit 9 — Dativ & Präpositionen
...
Change `expect(units).toHaveLength(8)` to `expect(units).toHaveLength(9)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(8)` to `expect(a1?.unitCount).toBe(9)`.
Change `expect(units).toHaveLength(8)` to `expect(units).toHaveLength(9)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 9 - Dativ & Präpositionen"
```

---

### Task 10: Seed A1 Unit 10 — Essen & Einkaufen
...
Change `expect(units).toHaveLength(9)` to `expect(units).toHaveLength(10)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(9)` to `expect(a1?.unitCount).toBe(10)`.
Change `expect(units).toHaveLength(9)` to `expect(units).toHaveLength(10)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 10 - Essen & Einkaufen"
```

---

### Task 11: Seed A1 Unit 11 — Perfekt (Einführung)
...
Change `expect(units).toHaveLength(10)` to `expect(units).toHaveLength(11)`.

- [ ] **Step 3: Update `tests/integration/learn.test.ts`**

Change `expect(a1?.unitCount).toBe(10)` to `expect(a1?.unitCount).toBe(11)`.
Change `expect(units).toHaveLength(10)` to `expect(units).toHaveLength(11)`.
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 11 - Perfekt (Einführung)"
```

---

### Task 12: Seed A1 Unit 12 — Imperativ & Wegbeschreibung (final unit)
...
- [ ] **Step 2: Update `tests/integration/seedContent.test.ts`, adding the final total-lesson-count assertion**
...
Change `expect(a1?.unitCount).toBe(11)` to `expect(a1?.unitCount).toBe(12)`.
Change `expect(units).toHaveLength(11)` to `expect(units).toHaveLength(12)`.
...
Run: `npx vitest run`
Expected: PASS — every test in the suite, including all Phase 1-4 tests plus this task's updates.

- [ ] **Step 6: Lint the whole project and commit**
...
```bash
git add prisma/seed.ts tests/integration/seedContent.test.ts tests/integration/learn.test.ts
git commit -m "feat: seed A1 unit 12 - Imperativ & Wegbeschreibung"
```
...
After Task 12, the A1 level has 12 units and 48 lessons (96 exercises, 96 vocab words across the 11 new units plus 2 for the new Begrüßung lesson). Run one more time to confirm the whole phase is coherent:
...
Both must be clean. This closes out the A1 portion of master design section 1's "A1 seviyesi tam içerikle" goal. A2/B1/B2 remain at their current 1-unit sample size — filling them to the same depth is a separate, future phase per `docs/superpowers/specs/2026-09-15-phase5-a1-curriculum-design.md` section 5 (Kapsam Dışı).
