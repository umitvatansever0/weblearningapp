...
describe('seed content', () => {
  ...
  it('has 13 A1 units with four lessons each', async () => {
  ...
  it('has 13 C2 units with four lessons each', async () => {
    const c2 = await prisma.level.findUniqueOrThrow({ where: { code: 'C2' } })
    const units = await prisma.unit.findMany({ where: { levelId: c2.id }, include: { lessons: true } })
    expect(units).toHaveLength(13)
    units.forEach((unit) => expect(unit.lessons).toHaveLength(4))
    const totalLessons = units.reduce((sum, unit) => sum + unit.lessons.length, 0)
    expect(totalLessons).toBe(52)
  ...
  it('has 13 B2 units with four lessons each', async () => {
    const b2 = await prisma.level.findUniqueOrThrow({ where: { code: 'B2' } })
    const units = await prisma.unit.findMany({ where: { levelId: b2.id }, include: { lessons: true } })
    expect(units).toHaveLength(13)
    units.forEach((unit) => expect(unit.lessons).toHaveLength(4))
    const totalLessons = units.reduce((sum, unit) => sum + unit.lessons.length, 0)
    expect(totalLessons).toBe(52)
  ...
  it('has 13 B1 units with four lessons each', async () => {
